...
from baccurate.extraction import SelectionPolicy
...
class EffectivePolicy:
    ...
    A selection policy is present exactly when the run must perform extraction, so a
...
def load_effective_policy(
    *,
    ...
    return EffectivePolicy(
        ...
        selection_policy=(
            SelectionPolicy.load(configuration_root / POLICY_FILENAMES[PolicySlot.SELECTION])
            if PolicySlot.SELECTION in required_policies
            else None
        ),
        host_policy=(
            HostPolicy.load(
